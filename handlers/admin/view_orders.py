    ...
    async with async_session() as session:
        ...
        for row in orders:
            ...
            kb = InlineKeyboardMarkup(inline_keyboard=[[
                ...
                InlineKeyboardButton(text="✅ Закрыта (удалить)", callback_data=f"status_done_{o['id']}")
            ]])
...
async def change_status(callback: types.CallbackQuery):
    ...
    async with async_session() as session:
        if action == "done":
            ...
            await callback.message.answer(f"🗑 Заказ #{order_id} удалён из базы (закрыт).")
        else:
            new_status = {"new": "новая", "work": "в работе"}[action]
            await session.execute(
